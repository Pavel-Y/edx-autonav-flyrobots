...
class UserCode:
    ...
    def calculatePredictMeasurementJacobian(self, x, marker_position_world, marker_yaw_world):
        ...
        c_psi = cos(x[2])
        s_psi = sin(x[2])
        delta_x = marker_position_world[0] - x[0]
        delta_y = marker_position_world[1] - x[1]
        H = np.array([[-c_psi, -s_psi, -s_psi*delta_x + c_psi*delta_y],
            [s_psi, -c_psi, -c_psi*delta_x - s_psi*delta_y],
            [0, 0, -1]])
